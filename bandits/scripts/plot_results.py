...
def plot_figure(data, x, y, filename, figsize=(24, 9), log_scale=False):
    ...
    g = sns.barplot(
        x=x, y=y, hue="Method", data=data, errwidth=2, ax=ax, palette=colors
    )
    if log_scale:
        g.set_yscale("log")
...
def plot_subspace_figure(df, filename=None):
    ...
    for line, method in zip(lines, labels):
        ...
        ax.fill_between(
            data["Subspace Dim"], y_lower_bound, y_upper_bound, color=color, alpha=0.3
        )
    ...
    file_path = (
        file_path + f"{dataset}_sub_reward.png"
        if filename is None
        else file_path + f"{filename}.png"
    )
...
colors = {
    k: sns.color_palette("Paired")[v]
    if k != "Lin-KF"
    else sns.color_palette("tab20")[8]
